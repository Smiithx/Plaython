...
async function seedDifficulties() {
    const data = [
        ...
        { label: "mid",    sort_order: 2 },
        { label: "hard",   sort_order: 3 },
        { label: "expert", sort_order: 4 },
    ];
    const { error } = await supabase
        .from("challenge_difficulties")
        .upsert(data, { onConflict: "label" });
    if (error) console.error("Error seeding difficulties:", error);
    else console.log("✅ Difficulties seeded");
}

async function seedStatuses() {
    const data = [
        { label: "ongoing",  sort_order: 1 },
        { label: "next",     sort_order: 2 },
        { label: "finished", sort_order: 3 },
    ];
    const { error } = await supabase
        .from("challenge_statuses")
        .upsert(data, { onConflict: "label" });
    if (error) console.error("Error seeding statuses:", error);
    else console.log("✅ Statuses seeded");
}
