...
export default function ChallengeDetailClient({ eventData }: ChallengeDetailClientProps) {
    const [activeTab, setActiveTab] = useState<"info" | "participants" | "schedule" | "discussion">("info");
    ...
    const calculateDuration = (): string => {
        ...
        if (diffDays > 0 && diffHours > 0) return `${diffDays} días y ${diffHours} horas`;
            ...
            <div className="container mx-auto px-4 py-8">
                <div className="flex flex-col lg:flex-row gap-8">
                    <div className="lg:w-2/3">
                            ...
                            <Tabs value={activeTab} onValueChange={setActiveTab} defaultValue="info">
                                <div className="px-6 pt-6">
                                    <TabsList className="grid grid-cols-4 gap-4 bg-gray-800 p-1 rounded-lg">
                                        ...
                                        <TabsTrigger value="discussion" className="rounded-md">
                                            Discusión
                                        </TabsTrigger>
                                    </TabsList>
                                </div>
                        ...
                        <Button onClick={handleJoinEvent} className="w-full py-3">
                            {isJoined ? (
                                <><CheckCircle className="mr-2" />Ya estás inscrito</>
                            ) : (
                                <><Zap className="mr-2" />Unirse al evento</>
                            )}
                        </Button>
                    </div>
